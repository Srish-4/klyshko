...
int main() {
    ...
    // Call the unified server setup and handshake function with the RA-TLS function pointer
    int ret = ssl_server_setup_and_handshake(ra_tls_create_key_and_crt_der_f);
    if (ret != 0) {
        printf("SSL server setup and handshake failed with error %d\n", ret);
    }
